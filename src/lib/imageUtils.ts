...
export function validateImageFile(file: File): { valid: boolean; error?: string } {
  ...
  if (file.size > IMAGE_CONFIG.maxFileSize) {
    return { valid: false, error: `File size too large. Maximum size is ${IMAGE_CONFIG.maxFileSize / (1024 * 1024)}MB.` };
  }
...
export function compressImage(file: File, quality: number = IMAGE_CONFIG.compressionQuality): Promise<string> {
...
export function generateThumbnail(dataUrl: string, size: number = IMAGE_CONFIG.thumbnailSize): Promise<string> {
  return new Promise((resolve, reject) => {
    const canvas = document.createElement('canvas');
    const ctx = canvas.getContext('2d');
    const img = new Image();
